...
const Forgot = ({ notify }) => {
    ...
    const pwdReset = async () => {
        ...
        try {
            const response = await fetch(import.meta.env.VITE_API_PATH + '/forgot-password', {
                method: 'POST',
            ...
            if (!response.ok) {
                notify('Linkin lähetys epäonnistui', 'error')
                return
            } else {
                setEmail('')
                notify('Salasanan palautuslinkki lähetetty', 'success')
                navigate('/')
            }

        } catch (error) {
            notify('Verkkovirhe tai palvelin ei vastaa', 'error')
        }
    }
